...
| P0 | Fix Part 2 P2-C4 — Replace pair-copy generator; authored local copy for area×service | 🟡 PARTIAL (**Fix Wave 26 — generator replaced**: 37 area + 15 suburb work profiles, no landmark salad, location-similarity **68.8% PASS**. URL keep-set / BP-1 phase 2 still ⏳ owner GSC — do not add/delete location pages) |
...
| P1 | §5.4-B1 — BM commercial tree (harga/problem/money templates) — largest content gap | ✅ COMPLETED 2026-08-30 (Fix Wave 24 — dedicated `/ms/harga` + `/zh/pricing` pricing hubs, 3-URL hreflang cluster with `/pricing`, `harga` 301 redirects, localized internal pricing links, sitemap update; see `FIX-WAVE-24-REPORT.md`) |
...
6. **BP-1 phase 2 / P2-C4 URL keep-set still blocked on owner GSC** — do **not**
   add more location pages; do **not** delete local pages on low traffic alone.
   **P2-C4 generator is done (Wave 26).**
...
10. **Next code-level candidates — ✅ VALUE QUEUE CLEARED IN WAVE 25
   ...
   **P3-12 phase 2** — ✅ **COMPLETE: 146/146 pods.** Waves 15–19 finished
   ...
   pages; **Wave 21 finished all 29 `commercial` pages** and **Wave 22
   finished all 29 `residential` pages** (both sourced from
   `commercialCopy` / `residentialCopy` in `config/content-pod-copy.ts`,
   editorial MS/ZH, no MT), and both `commercial` and `residential` are now
   completed, permanently gated families. Together: **1168 native MS/ZH Q&As,
...
| P1 | P4-13 — Restructure the local pages (site-wide price + parametric body) | 🟡 PARTIAL (**Fix Wave 26 — parametric body rewritten**, no salad. URL inventory still **BP-1 phase 2**, owner GSC keep-set) |
