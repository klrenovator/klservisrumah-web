...
/* ── AI-context files are in sync with the published data ────────────────
   `public/llms.txt`, `llms-full.txt`, `aeo-faq.txt` and `site-summary.json`
   are what answer engines read instead of the pages. They are generated by
   `npm run gen:ai-context`, but they are also committed, so a hand-edit (or a
   config change made without re-running the generator) could put a wrong
   price or a dead social link in front of ChatGPT/Perplexity while every
   rendered page stays correct — which is exactly how they previously ended up
   quoting five-year-old prices and a competitor's Instagram.

   These assertions re-check the committed files against the live config. */
console.log("\n• AI-context files (llms.txt, llms-full.txt, aeo-faq.txt, site-summary.json)");
{
  ...
  const aeoFaq = readPublic("aeo-faq.txt");
  const summary = JSON.parse(readPublic("site-summary.json")) as {
    profiles: Record<string, string>;
    services: { slug: string; startPrice: string; warranty: string }[];
  ...
  for (const key of ["facebook", "instagram", "googleBusiness"] as const) {
    ...
    assert(
      ...
      `AI context: ${key} link is stale — expected ${expected}. Run: npm run gen:ai-context`
    );
    assert(
      summary.profiles[key] === expected,
      `site-summary.json: ${key} is "${summary.profiles[key]}", expected "${expected}"`
    );
  }

  // 2. No link to a profile that was replaced. These short links belonged to a
  //    different brand's presence and must never resurface in AI-facing copy.
  for (const dead of [
    "instagram.com/klrenovator",
    "facebook.com/share/1DDDB3523A",
    "maps.app.goo.gl/dG5WWYBCotRQzvRJA",
    "share.google/HhXvqWDkefZ5bzNdL"
  ]) {
    assert(!corpus.includes(dead), `AI context: retired link "${dead}" is still published`);
  }

  // 3. Published starting price + warranty per service must match exactly.
  assert(
    summary.services.length === Object.keys(servicesData).length,
    `site-summary.json lists ${summary.services.length} services, config has ${Object.keys(servicesData).length}`
  );
  for (const record of summary.services) {
    const service = servicesData[record.slug];
    assert(Boolean(service), `site-summary.json: unknown service "${record.slug}"`);
    if (!service) continue;
    assert(
      record.startPrice === service.startPrice,
      `site-summary.json: ${record.slug} price "${record.startPrice}" ≠ published "${service.startPrice}"`
    );
    assert(
  ...
  // 4. Counts must match the config, so an assistant never states a stale scale.
  assert(
    summary.counts.services === Object.keys(servicesData).length,
    `site-summary.json: services count ${summary.counts.services} ≠ ${Object.keys(servicesData).length}`
  );
  assert(
    summary.counts.areaPages === areaPages.length,
    `site-summary.json: areaPages ${summary.counts.areaPages} ≠ ${areaPages.length}`
  );
  assert(
    summary.counts.problemPages === problemPages.length,
    `site-summary.json: problemPages ${summary.counts.problemPages} ≠ ${problemPages.length}`
  );

  // 5. Contact details must be the live ones.
  for (const value of [siteConfig.phoneDisplay, siteConfig.email]) {
    assert(corpus.includes(value), `AI context: "${value}" missing — files are out of date`);
  }

  if (failures === failuresBefore) {
    console.log(
      `  ✓ 4 AI-context files verified — ${summary.services.length} prices, ${summary.services.length} warranties, 5 profile links and all counts match the published data`
    );
  } else {
    console.error("  → Run `npm run gen:ai-context` to regenerate them from the published data.");
  }
}
