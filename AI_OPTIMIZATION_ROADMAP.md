...
1. **Problem-page body translations are COMPLETE: all 77 problem pages now have native MS + ZH bodies** (43 original + 34 from `problem-data-extra.ts`). The `LocaleProblemView` component renders these translations when users browse in MS or ZH mode. The `RelatedSpecialtyContent` component on MS/ZH specialty pages now shows localized problem titles and symptoms. The next in-repo milestone is **real locale problem routes (154)** — still blocked pending evidence-based consolidation of the 14 problem-overlap groups (GSC data); do not force it. Until then, prefer the non-blocked backlog: translate the remaining `problemI18n` overrides in `config/problem-data.ts` (these are the shorter i18n overrides for the original 43 problems — some may already have overrides, check `problemI18n` in `config/problem-data.ts` for gaps), or improve the `RelatedProblems` client component (`components/sections/related-problems.tsx`) to show localized content when the site is in MS/ZH mode (it currently shows English titles/symptoms since it's a client component using raw `problemPages` data).
2. Add real locale problem routes (154) only after the 14 problem-overlap groups are consolidated with evidence; then localize `RelatedProblems` links on localized specialty pages (they currently point to EN problem pages—documented and still blocked).
3. Obtain business/translator QA for the completed localized specialty pages (**224 so far — the full catalogue**) and problem-page bodies (**77 × MS + ZH = 154 translated bodies**) before measuring GSC indexation; this requires an owner/translator and cannot be completed from repository code alone. After QA + deploy, re-measure GSC/Bing coverage of the localized trees and the MS/ZH pilot conversions before the owner-only H3 full-rollout decision.
4. Do not create thin or spun pages; rerun lint, type-check, build, topical-map, specialty-locale, i18n, HTML and SEO audits after any future change.
