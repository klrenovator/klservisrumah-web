...
- 🟡 **PHASE 14 — SITEMAP**
  - Existing sitemap: **3,400 unique URLs after the CCTV tranche**, including all 104 real MS/ZH specialty pages, with 0 duplicate `<loc>` values.
...
- ✅ **PHASE 20 — QUALITY CONTROL** *(2026-08-10)*
  - TypeScript PASS; production build PASS (`audit:html` checks **4,439 pages**, 0 fatal / 0 warnings); i18n PASS (**1,074 keys × 3**); 263,293 estimator/content assertions PASS; npm audit 0 vulnerabilities; `npm run seo:audit` PASS.
...
- **MS service-tree URLs:** 81 (index + 28 main + 52 authored specialties)
- **ZH service-tree URLs:** 81 (index + 28 main + 52 authored specialties)
...
1. **Do not create Air Conditioning pages or content.** The owner confirmed standalone Air Conditioning is not a KL Servis Rumah service. CCTV is now complete, bringing the rollout to **52 specialties × MS + ZH (104 localized pages)**. Pick the next coherent four-specialty tranche from an actual remaining high-value service—prefer **Glass & Aluminium / Window Repair**, then Door or Roofing—and reuse `specialty-locale-content.ts` + `LocaleSpecialtyPage` with genuine native MS/ZH content.
...
3. Obtain business/translator QA for the 104 completed localized specialty pages before measuring GSC indexation; this requires an owner/translator and cannot be completed from repository code alone.
4. Do not create thin or spun pages; rerun lint, type-check, build, topical-map, specialty-locale, i18n, HTML and SEO audits after every tranche.
