  ...
  return (
    <section className="bg-gradient-to-b from-slate-50 to-white py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <span className="rounded-full bg-[#E0F2FE] px-4 py-1.5 text-xs font-extrabold uppercase tracking-widest text-[#0284C7]">Problem guides</span>
        <h1 className="mt-4 text-3xl font-extrabold text-[#075985] sm:text-5xl">Find the cause, fix and cost range</h1>
          ...
          {problemPages.map((problem) => {
            ...
            return (
              <Link key={problem.slug} href={`/problems/${problem.slug}`} className="rounded-3xl border border-slate-100 bg-white p-5 shadow-xs transition hover:-translate-y-0.5 hover:shadow-md">
                ...
                <h2 className="mt-2 text-lg font-extrabold text-[#075985]">{problem.title}</h2>
