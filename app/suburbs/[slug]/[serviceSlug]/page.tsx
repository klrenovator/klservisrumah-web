      ...
      <section className="bg-gradient-to-b from-slate-50 to-white py-16 sm:py-20">
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-4 sm:px-6 lg:grid-cols-12 lg:px-8">
          <div className="lg:col-span-8 flex flex-col gap-8">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full bg-[#E0F2FE] px-4 py-1.5 text-xs font-extrabold uppercase tracking-widest text-[#0284C7]"><MapPin className="h-4 w-4" /> {suburb.name}</span>
              <h1 className="mt-4 text-3xl font-extrabold text-[#075985] sm:text-5xl">{service.title} in {suburb.name} — From {service.startPrice}</h1>
            ...
            <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-xs sm:p-8">
              <h2 className="text-2xl font-extrabold text-[#075985]">Local landmarks covered</h2>
            ...
            <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-xs sm:p-8">
              <h2 className="text-2xl font-extrabold text-[#075985]">Pricing guide</h2>
              <div className="mt-5 space-y-3">{service.subServices.map((sub) => <div key={sub.name} className="rounded-2xl bg-slate-50 p-4"><h3 className="font-extrabold text-[#075985]">{sub.name}</h3><p className="text-sm font-bold text-[#0EA5E9]">{sub.price}</p><p className="mt-1 text-xs font-semibold text-[#475569]">{sub.desc}</p></div>)}</div>
            </div>

            <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-xs sm:p-8">
              <h2 className="mb-5 text-2xl font-extrabold text-[#075985]">Nearby suburb pages</h2>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{suburb.nearbySuburbs.map((nearby) => <Link key={nearby} href={`/suburbs/${nearby}/${service.slug}`} className="rounded-2xl bg-slate-50 p-4 text-sm font-extrabold text-[#075985] hover:bg-[#E0F2FE]">{nearby.replace(/-/g, " ")} {service.title}</Link>)}</div>
            </div>

            <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-xs sm:p-8"><h2 className="mb-5 text-2xl font-extrabold text-[#075985]">FAQs</h2><div className="space-y-4">{faqs.map((faq) => <div key={faq.q} className="rounded-2xl bg-slate-50 p-4"><h3 className="font-extrabold text-[#075985]">{faq.q}</h3><p className="mt-2 text-sm font-semibold text-[#475569]">{faq.a}</p></div>)}</div></div>
          </div>
