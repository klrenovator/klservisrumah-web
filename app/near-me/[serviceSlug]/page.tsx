    ...
    <>
      ...
      <section className="bg-gradient-to-b from-slate-50 to-white py-16 sm:py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <span className="inline-flex items-center gap-2 rounded-full bg-[#E0F2FE] px-4 py-1.5 text-xs font-extrabold uppercase tracking-widest text-[#0284C7]"><MapPinned className="h-4 w-4" /> Near me</span>
          <h1 className="mt-4 text-3xl font-extrabold text-[#075985] sm:text-5xl">Find {service.title} Near You in KL & Selangor</h1>
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {areaPages.map((area) => (
              <Link key={area.slug} href={`/areas/${area.slug}/${service.slug}`} className="rounded-3xl border border-slate-100 bg-white p-5 shadow-xs transition hover:-translate-y-0.5 hover:shadow-md">
                <h2 className="text-lg font-extrabold text-[#075985]">{service.title} in {area.name}</h2>
          ...
          <div className="mt-10 rounded-3xl border border-slate-100 bg-white p-6 shadow-xs">
            <h2 className="text-2xl font-extrabold text-[#075985]">Popular suburbs</h2>
            <div className="mt-5 flex flex-wrap gap-3">
              {suburbPages.slice(0, 24).map((suburb) => <Link key={suburb.slug} href={`/suburbs/${suburb.slug}/${service.slug}`} className="rounded-full bg-slate-50 px-4 py-2 text-xs font-bold text-[#475569] hover:bg-[#E0F2FE]">{suburb.name}</Link>)}
            </div>
          </div>
