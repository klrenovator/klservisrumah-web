          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="lg:col-span-5 flex flex-col gap-8">
              <div className="flex flex-col gap-4">
                <span className="text-xs font-bold text-[#0781B2] uppercase tracking-widest bg-[#0781B2]/10 px-4.5 py-1.5 rounded-full w-fit">
                  Get In Touch
                </span>
                <h1 className="text-3xl sm:text-4xl font-extrabold text-[#021F44] tracking-tight">
                  Book Your Service
                </h1>
                <p className="text-sm sm:text-base text-[#4A607C] leading-relaxed font-semibold">
              ...
              <div className="flex flex-col gap-5">
                <div className="flex gap-4 items-start p-4 bg-white rounded-2xl border border-slate-100 shadow-3xs">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-xs sm:text-sm text-[#021F44] font-bold leading-relaxed">{siteConfig.address}</span>
                  </div>
                </div>

                <div className="flex gap-4 items-start p-4 bg-white rounded-2xl border border-slate-100 shadow-3xs">
                  <Phone className="w-5 h-5 text-[#0781B2] shrink-0 mt-0.5" />
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Call Dispatch Support</span>
                    <a href={`tel:${siteConfig.phone}`} className="text-xs sm:text-sm text-[#0781B2] font-extrabold">{siteConfig.phoneDisplay}</a>
                  </div>
                </div>

                <div className="flex gap-4 items-start p-4 bg-white rounded-2xl border border-slate-100 shadow-3xs">
                  <Mail className="w-5 h-5 text-[#0781B2] shrink-0 mt-0.5" />
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Email Inquiry Desk</span>
                    <a href={`mailto:${siteConfig.email}`} className="text-xs sm:text-sm text-[#021F44] font-bold">{siteConfig.email}</a>
                  </div>
                </div>

                <div className="flex gap-4 items-start p-4 bg-white rounded-2xl border border-slate-100 shadow-3xs">
                  <Clock className="w-5 h-5 text-[#0781B2] shrink-0 mt-0.5" />
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Operating Schedule</span>
                    <span className="text-xs sm:text-sm text-[#021F44] font-bold">{siteConfig.hours}</span>
                  </div>
                </div>
              </div>

              <div className="bg-[#F2F6FC] rounded-3xl p-5 border border-slate-100/60 flex gap-3">
                <ShieldAlert className="w-5 h-5 text-[#0781B2] shrink-0 mt-0.5" />
                <p className="text-[11px] font-bold text-[#4A607C] leading-normal">
