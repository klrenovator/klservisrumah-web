          ...
          <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-3">
            {["Stop or isolate the source if safe", "Send photos and your location", "Approve the quote before repair starts"].map((step, idx) => (
              <div key={step} className="rounded-3xl border border-slate-100 bg-white p-6 shadow-xs">
                <span className="rounded-xl bg-[#E0F2FE] px-3 py-1.5 text-sm font-extrabold text-[#0284C7]">0{idx + 1}</span>
                <h2 className="mt-4 text-lg font-extrabold text-[#075985]">{step}</h2>
              </div>
            ))}
          </div>

          <div className="mt-10 rounded-3xl border border-slate-100 bg-white p-6 shadow-xs sm:p-8">
            <h2 className="flex items-center gap-2 text-2xl font-extrabold text-[#075985]"><Clock3 className="h-6 w-6 text-[#0EA5E9]" /> Fastest coverage areas</h2>
