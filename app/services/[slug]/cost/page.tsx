        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 overflow-x-auto rounded-3xl border border-slate-100 bg-white shadow-xs">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-widest text-[#075985]"><tr><th className="px-5 py-4">Scope</th><th className="px-5 py-4">Our published rate</th><th className="px-5 py-4">Market range</th><th className="px-5 py-4">Notes</th></tr></thead>
              <tbody className="divide-y divide-slate-100">
                {(rates.length ? rates : service.subServices.map((sub) => ({ label: sub.name, publishedRate: sub.price, marketRange: "Confirmed after scope", validationNote: sub.desc }))).map((rate) => (
                  <tr key={rate.label}><td className="px-5 py-4 font-extrabold text-[#075985]">{rate.label}</td><td className="px-5 py-4 font-bold text-[#0EA5E9]">{rate.publishedRate}</td><td className="px-5 py-4 text-[#475569]">{rate.marketRange}</td><td className="px-5 py-4 text-xs font-semibold text-[#475569]">{rate.validationNote}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
