...
> **Fix Wave 16 tranche 2 COMPLETE (2026-08-30); P3-12 phase 2 remains ⏳:**
> all 10 `guides` pods now have native editorial MS/ZH translations of their
> four authored English FAQs (**80 localized Q&As**). Together with Wave 15's
> `top` family, the permanent gate reports **20/146 pods and 160 localized
> Q&As**; completed families are `top` and `guides`. Production build: 4,139
> HTML; direct probe: 20/20 MS/ZH guide pages and 80/80 Q&As visible and
> byte-identical to FAQPage schema. Full corpus gates pass. **Exact next stop:
...
>
> **NEXT (P3-12 phase 2, tranche 3):** native MS/ZH editorial translation
> of all 10 authored `guidesMaintenance` FAQ sets. Start in `maintenanceCopy`
> inside `config/content-pod-copy-batch2.ts`; add `guidesMaintenance:<slug>`
> records to `CONTENT_POD_FAQ_I18N`; add `guidesMaintenance` to
> `COMPLETED_FAMILIES` only after all 10 pass. Phase status is 20/146, not
