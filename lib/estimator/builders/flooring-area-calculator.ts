...
export function buildFlooringAreaCalculatorSpec(t: Translator): EstimatorSpec {
  const includeWalls = true;
  const includeCeiling = true;
  const hasHeight = true;
  const hasSlope = false;
  return {
    ...
    compute: (answers) => {
      ...
      return {
        ...
        addOns: [],
        related: [
          { label: t("related.service.label"), href: "/services/flooring", desc: t("related.service.desc") },
