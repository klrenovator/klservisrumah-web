    ...
    {open && <div className="fixed inset-0 z-[80]" role="dialog" aria-modal="true" aria-label="Main navigation">
      ...
      <aside className="absolute inset-y-0 right-0 flex w-[min(22rem,88vw)] flex-col bg-white shadow-2xl animate-in slide-in-from-right duration-300">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <div><p className="text-base font-extrabold text-[#075985]">Menu</p><p className="mt-0.5 text-xs text-slate-500">KL Servis Rumah</p></div>
          <button type="button" onClick={() => setOpen(false)} aria-label="Close menu" className="rounded-xl border border-slate-200 p-2 text-slate-600 transition hover:bg-slate-50"><X className="h-5 w-5" /></button>
        </div>
        <nav className="flex-1 px-4 py-5" aria-label="Main pages">
          <p className="px-3 pb-2 text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Main pages</p>
