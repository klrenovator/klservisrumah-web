...
export function EstimateResultPanel({
  ...
  // ── Send to Customer Feature ────────────────────────────────────────
  const [showSendPanel, setShowSendPanel] = useState(false);
  const [customerName, setCustomerName] = useState("");
  const [customerPhone, setCustomerPhone] = useState("");
  const [messageSent, setMessageSent] = useState(false);

  // Build detailed estimate message for customer
  const customerEstimateMessage = useMemo(() => {
    const chosen = result.addOns.filter((addOn) => selectedAddOns.includes(addOn.id));
    const date = new Date().toLocaleDateString('en-MY', { day: '2-digit', month: 'short', year: 'numeric' });
    
    const lines = [
      `📋 *ESTIMATE FOR: ${customerName || 'Customer'}*`,
      `📅 Date: ${date}`,
      ...
      `⏱️ *Estimated Time:* ${result.duration}`,
      ``,
      `━━━━━━━━━━━━━━━━━━━━`,
      `💵 *PRICE BREAKDOWN*`,
      `━━━━━━━━━━━━━━━━━━━━`,
      `   Labour: ${formatMYR(result.labour)}`,
      `   Materials: ${formatMYR(result.materials)}`,
      `   ─────────────`,
      `   Base Estimate: ${formatMYR(result.price)}`,
      `   Range: ${formatMYR(result.low)} – ${formatMYR(result.high)}`,
    ];

    if (chosen.length > 0) {
      lines.push(``);
      lines.push(`➕ *Add-ons:*`);
      chosen.forEach((addOn) => {
        lines.push(`   • ${addOn.label}: +${formatMYR(addOn.price)}`);
      });
      lines.push(`   ─────────────`);
      lines.push(`   *Add-ons Total: ${formatMYR(addOnTotal)}*`);
    }

    lines.push(``);
    lines.push(`━━━━━━━━━━━━━━━━━━━━`);
    lines.push(`✨ *TOTAL ESTIMATED COST: ${formatMYR(grandTotal)}*`);
    lines.push(`📊 *Range: ${formatMYR(grandLow)} – ${formatMYR(grandHigh)}*`);
    lines.push(`💡 *Note: Incl. labour & materials*`);
    lines.push(`━━━━━━━━━━━━━━━━━━━━`);
    lines.push(``);
    lines.push(`📝 *What's Included:*`);
    lines.push(`   ✓ Upfront fixed pricing`);
    lines.push(`   ✓ Background-verified team`);
    lines.push(`   ✓ Professional materials`);
    lines.push(`   ✓ Clean handover`);
    lines.push(`   ✓ Written warranty`);
    lines.push(``);
    lines.push(`⚠️ *Disclaimer:*`);
    lines.push(`This is an estimate. Final price confirmed`);
    lines.push(`after on-site inspection. T&Cs apply.`);
    lines.push(``);
    lines.push(`━━━━━━━━━━━━━━━━━━━━`);
    lines.push(`📞 *Ready to book?*`);
    lines.push(`WhatsApp us: ${siteConfig.phoneDisplay}`);
    lines.push(`🌐 ${siteConfig.url}`);
    lines.push(``);
    lines.push(`_Generated by KL Servis Rumah Estimator_`);

    return lines.join("\n");
  }, [customerName, result, summaryLines, grandTotal, grandLow, grandHigh, selectedAddOns, addOnTotal]);
  ...
  const handleSendEstimate = () => {
    ...
    setMessageSent(true);
    trackEvent({
      action: "send_estimate_to_customer",
      category: "lead",
      label: toolName,
      value: grandTotal
    });
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Send Estimate to Customer (Business Owner Feature) ─────────── */}
      <section className="rounded-3xl border-2 border-violet-200 bg-gradient-to-br from-violet-50 to-white p-6 sm:p-7 shadow-lg">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            className={`inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-black transition ${
        ...
        {showSendPanel && (
          <div className="mt-6 space-y-5">
            ...
            <div className="flex flex-wrap gap-3">
              <a
                href={`https://wa.me/${customerPhone ? customerPhone.replace(/\D/g, '') : ''}?text=${encodeURIComponent(customerEstimateMessage)}`}
                target="_blank"
                rel="noopener noreferrer"
